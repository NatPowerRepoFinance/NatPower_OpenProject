...
class Projects::Settings::PdaNfs::LandTitlesController < Projects::Settings::PdaNfsController
  skip_before_action :authorize, only: %i[new create show edit update destroy]
  before_action :find_pda_nf_and_land_parcel
  ...
  def show
    @land_title = @land_parcel.land_titles_nfs.find_by!(land_title_id: params[:id])
  end
  ...
  def update
    ...
    if @land_title.update(land_title_params)
      ...
      redirect_to project_settings_pda_nf_land_parcel_land_title_path(@project, @pda_nf, @land_parcel, @land_title)
    else
  ...
  def find_pda_nf_and_land_parcel
